...
for (my_gene in strict_mes_1375) {
  ...
  tryCatch({
    ...
    partition_vector <- c(partition_vector, as.numeric(partitions_gene[2,2]))
    ...
    summary.survfit_object_500_gene <- assign(paste0("summary_survfit_object_500_",  my_gene),   
                                              summary(fit_my_gene_smallest_pval, times = 500))
    
    summary_survfit_object_500   <- assign(paste0("summary_survfit_object_",  my_gene, "_500"),   
                                           summary(fit_my_gene_smallest_pval, times = 500))
    summary_survfit_object_1000  <- assign(paste0("summary_survfit_object_",  my_gene, "_1000"),  
                                           summary(fit_my_gene_smallest_pval, times = 1000))
    summary_survfit_object_2000  <- assign(paste0("summary_survfit_object_",  my_gene, "_2000"),  
                                           summary(fit_my_gene_smallest_pval, times = 2000))
    ...
    print(paste0("Calculate surv prob 500 days for gene ", my_gene))
    surv_prob_1_500_vector  <- c(surv_prob_1_500_vector,  summary.survfit_object_500$surv[1] )
    print(paste0("surv_prob_1_500_vector[1] is ", surv_prob_1_500_vector[1], " for gene ", my_gene))
    print(paste0("surv_prob_1_500_vector[2] is ", surv_prob_1_500_vector[2], " for gene ", my_gene))
    
    print(paste0("Calculate High and Low surv prob 500 days vectors for gene ", my_gene))
    surv_prob_1_500_vector_high  <- c(surv_prob_1_500_vector_high,  summary.survfit_object_500$surv[1] )
    surv_prob_1_500_vector_low   <- c(surv_prob_1_500_vector_low,   summary.survfit_object_500$surv[2] )
    
    surv_prob_1_500_high_gene    <- assign(paste0("surv_prob_1_500_high_" ,  my_gene), summary.survfit_object_500$surv[1])
    surv_prob_1_500_low_gene     <- assign(paste0("surv_prob_1_500_low_"  ,  my_gene), summary.survfit_object_500$surv[2])
    
    surv_prob_2_500_vector  <- c(surv_prob_2_500_vector,  summary.survfit_object_500$surv[2] )
    print(paste0("Calculate surv prob 1000 days for gene ", my_gene))
    surv_prob_1_1000_vector <- c(surv_prob_1_1000_vector, summary.survfit_object_1000$surv[1])
    surv_prob_2_1000_vector <- c(surv_prob_2_1000_vector, summary.survfit_object_1000$surv[2])
    print(paste0("Calculate surv prob 2000 days for gene ", my_gene))
    surv_prob_1_2000_vector <- c(surv_prob_1_2000_vector, summary.survfit_object_2000$surv[1])
    surv_prob_2_2000_vector <- c(surv_prob_2_2000_vector, summary.survfit_object_2000$surv[2])
    
    # Prepare surv prob vector using the assign function
    surv_prob_1_500_vector_assign  <- assign(paste0("surv_prob_1_500_vector_" ,  my_gene),  
                                             summary(fit_my_gene_smallest_pval, times = 500))
    surv_prob_2_500_vector_assign  <- assign(paste0("surv_prob_1_500_vector_" ,  my_gene),  
                                             summary(fit_my_gene_smallest_pval, times = 500))
    ...
    mes_high_surv_prob_assigned <- assign(paste0("mes_high_surv_prob_", my_gene), 
                                          summary.survfit_object_500$surv[1])
    mes_low_surv_prob_assigned  <- assign(paste0("mes_low_surv_prob_",  my_gene),  
                                          summary.survfit_object_500$surv[2])
    
    if (mes_high_surv_prob < mes_low_surv_prob) {
      
      assign(paste0("surv_prob_gene_high_smaller_gene_low_",my_gene))
      gene_expression_vector_surv_prob_smaller   <- c(gene_expression_vector_surv_prob_smaller, mes_high_surv_prob)
      my_gene_vector_surv_prob_smaller           <- c(my_gene_vector_surv_prob_smaller, my_gene)
      
    }
    
    if (mes_high_surv_prob > mes_low_surv_prob) {
      
      print("surv_prob_gene_high_bigger_gene_low_", my_gene)
      gene_expression_vector_surv_prob_bigger    <- c(gene_expression_vector_surv_prob_bigger, mes_high_surv_prob)
      my_gene_vector_surv_prob_bigger            <- c(my_gene_vector_surv_prob_bigger, my_gene)
...
tmp_partition <- cbind(gene_vector, partition_vector)
...
write.table(tmp_pvalue, file = paste0("../data/", "strict_mes_1375.txt"),
            sep = "\t",
            row.names = F,
            col.names = T,
            quote = F)

tmp_surv_prob_500_vector                      <- cbind(surv_prob_1_500_vector,   surv_prob_2_500_vector)
tmp_surv_prob_1000_vector                     <- cbind(surv_prob_1_1000_vector,  surv_prob_2_1000_vector)
tmp_surv_prob_500_and_1000_vector             <- cbind(tmp_surv_prob_500_vector, tmp_surv_prob_1000_vector)
tmp_surv_prob_2000_vector                     <- cbind(surv_prob_1_2000_vector,  surv_prob_2_2000_vector)

tmp_surv_prob_500_1000_and_2000_vector        <- cbind(tmp_surv_prob_500_and_1000_vector, tmp_surv_prob_2000_vector)

gene_partition_pvalue_mtx                     <- cbind(tmp_partition, smallest_pval_vector)

gene_partition_surv_prob_500                  <- as.data.frame(gene_partition_pvalue_mtx, tmp_surv_prob_500_vector)
# gene_partition_surv_prob_500_df               <- cbind(gene_partition_pvalue_mtx, tmp_surv_prob_500_vector)


gene_partition_surv_prob_500_1000_2000_df     <- as.data.frame(gene_partition_pvalue_mtx, tmp_surv_prob_500_1000_and_2000_vector)

gene_partition_surv_prob_500_1000_2000_df     <- gene_partition_surv_prob_500_1000_2000_df[order(smallest_pval_vector),] 

write.table(gene_partition_surv_prob_500_1000_2000_df, file = paste0("../data/", "gene_partition_surv_prob_500_1000_2000_df_gardner.txt"),
            sep = "\t",
            row.names = F,
            col.names = F,
            quote = F)

write.table(gene_partition_surv_prob_500_1000_2000_df, file = paste0("../data/", "strict_mes_1375_gene_partition_surv_prob_500_1000_2000_df.txt"),
            sep = "\t",
            row.names = F,
            col.names = T,
            quote = F)